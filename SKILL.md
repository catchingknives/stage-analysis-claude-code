---
name: StageAnalysis
description: Stan Weinstein Stage Analysis on any ticker. Determines current stage (1-4), computes moving averages, volume ratios, relative strength, and generates actionable buy/sell signals. USE WHEN stage analysis, what stage, weinstein, technical analysis, stage check, analyze ticker, check stage.
---

## Customization

**Before executing, check for user customizations at:**
`~/.claude/skills/PAI/USER/SKILLCUSTOMIZATIONS/StageAnalysis/`
...
1. **Send voice notification**:
   ...
   curl -s -X POST http://localhost:8888/notify \
     ...
     -d '{"message": "Running Stage Analysis on the requested tickers", "voice_id": "fTtv3eikoepIosk8dTZ5", "voice_enabled": true}' \
     > /dev/null 2>&1 &
   ```

2. **Output text notification**:
   ```
   Running **StageAnalysis** on {TICKERS}...
   ```
...
`~/.claude/skills/StageAnalysis/Methodology.md`
