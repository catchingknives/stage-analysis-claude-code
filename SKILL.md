...
- **All entries expire after 4 hours:** every fetch includes the still-forming weekly candle, so no entry is ever safe to keep permanently
- **Cache key:** `{TICKER}_{YYYY}_W{WW}_current.json` (keyed by current ISO week)
