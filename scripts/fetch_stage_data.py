...
def get_cache_path(ticker, now=None):
    """
    Return the cache file path for a ticker, keyed by ISO week.

    Every entry is a "_current" entry and always carries the CACHE_TTL_HOURS
    expiry: a fetch always includes the still-forming weekly candle, so no
    entry is ever safe to keep permanently. (An earlier version compared
    now.isocalendar() against itself, so its "completed week, cache forever"
    branch was dead code; honoring it would have served a non-final candle
    as if final, so the branch was removed rather than fixed.)
    """
    if now is None:
        now = datetime.now()
...
def fetch_and_compute(ticker_symbol, include_spx=False, no_cache=False):
    ...
    # Weekly gaps (institutional interest change; gap-up + heavy volume on a
    # breakout week is a top-tier buy signal, see Methodology "Gap-Up Breakout")
    if l > ph:
        candle_patterns.append("gap_up")
    if h < pl:
        candle_patterns.append("gap_down")
...
def classify_stage(price, ema_30w, ema_10w, slope_30w, slope_10w,
    ...
    RS is corroboration only: it annotates the detail string when it disagrees
    with the price-structure stage call, but never changes the stage number.
    """
    stage, detail = _classify_price_structure(
        price, ema_30w, ema_10w, slope_30w, slope_10w,
        vol_ratio, above_30w, above_10w, pct_from_high)

    if rs_data:
        rs_weak = (rs_data.get("rs_above_40w_ma") is False
                   or rs_data.get("rs_direction") == "falling")
        if stage == 2 and rs_weak:
            detail += " | RS NOT confirming (below 40w MA or falling)"
        elif stage in (1, 2) and rs_data.get("rs_above_40w_ma") and rs_data.get("rs_direction") == "rising":
            detail += " | RS confirming (above 40w MA and rising)"
        elif stage == 3 and rs_data.get("rs_direction") == "falling":
            detail += " | RS declining supports the topping read"

    return stage, detail


def _classify_price_structure(price, ema_30w, ema_10w, slope_30w, slope_10w,
                              vol_ratio, above_30w, above_10w, pct_from_high):
    """Price/MA/volume structure classification (RS-blind)."""
    # Thresholds
